...
+ 说明
    ...
    + 通过JSON.parse()方法可以将一个JSON串转换成对象(注意最后一个不要有逗号)
...
```java
 @Test
    public void test01(){
        //实例化User对象,将User对象转换为JSON
        User user = new User(1,"张三","666");
        //将Person对象转换为一个字符串 Gson Jackson Fastjson

        //将一个对象转换为json
        String json = JSON.toJSONString(user);
        System.out.println(json);//{"id":1,"password":"666","username":"张三"}

        //JSON字符串转java对象
        User user2 = JSON.parseObject(json,User.class);
        System.out.println(user2);//User(id=1, username=张三, password=666)
    }
...
| [onchange](https://www.runoob.com/jsref/event-onchange.html) | 该事件在表单元素的内容改变时触发(失去焦点时)( <input>, <keygen>, <select>, 和 <textarea>) |
...
```js
    var flag = confirm("信息确认框")//返回boolean
    prompt("信息输入框")
    alert("信息提示框")
```

...
public class ServletB extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        //迭代cookies数组  如果没有cookie那么他为null
...
a form标签请求,客户端操作等待服务端,服务端响应，客户端等待。(一条时间线上)

异步交互:

客户端操作过程中可以发送请求,不中断后续操作。无需等待,可以连续发多个请求。
    ...
    xmlhttp.onreadystatechange=function(){
      ...
      if (xmlhttp.readyState==4 && xmlhttp.status==200)
      {
        document.getElementById("myDiv").innerHTML=xmlhttp.responseText;
          //window.location.href="http://139.9.114.22" 页面跳转
      }
    }
      // 设置请求方式和请求的资源路径
...
下面检验用户名的代码有一个问题,用户名不合法不应该提交,但表单还是提交了,将来vue和promise会解决这个问题，`return false`没生效
...
<head>
    ...
    <script>
        ...
        function checkUsername(){
            ...
            request.send();
            // 前面校验都通过
            // usernameMsgSpan.innerText="OK"  这两行都废掉了，因为
            // return true

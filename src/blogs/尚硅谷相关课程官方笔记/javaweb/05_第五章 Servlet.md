...
public class UserServlet  extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        resp.setHeader("Context-Type","text/html");//设置响应格式
        resp.setContentType("text/html");//专门的api
...
@WebServlet(
        ...
        initParams = {@WebInitParam(name = "encoding",value = "UTF-8")},//init-param
        loadOnStartup = 6//第几个实例化
)
public class UserServlet  extends HttpServlet {
    @Override
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
...
+ abstract class HttpServlet extends GenericServlet  HttpServlet抽象类,除了基本的实现以外,增加了更多的基础功能
    ...
    + protected void doTrace(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException
        ...
        + 不重写service的话就会调用父类的service故意响应错误信息报405请求方式不允许
...
+ 自定义Servlet中,必须要对处理请求的方法进行重写(不重写的话就会报405请求方式不允许)
    + 要么重写service方法
    + 要么重写doGet/doPost方法
    
    
    
    + 后续使用了springMVC框架后,我们则无需继承HttoServlet,处理请求方法也无需是do*** service
    + 如果doget和dopost方法中,我们定义的代码都一样,可以让一个方法直接调用另一个就行了
...
以get方式请求,参数以键值对形式放在url里,不放在请求体里,ge方式的请求也是可以有请求体的
...
+ 方式2 : 设置Tomcat10.1.7的URI解析字符集和GET请求发送时所使用URL转码时的字符集一致即可,修改conf/server.xml中 Connecter 添加 URIEncoding="GBK"  (不推荐)URIEncoding对请求头起作用,请求体无效(post)
...
当前页面中,所有不加任何修饰的相对路径前,会自动补充href中的内容。(不完美)
...
请求转发不需要添加项目上下文  / 代表的路径是 `http://localhost:8080/项目名字/`
